...
const breadcrumbs: BreadcrumbItem[] = [{ title: 'Two-Factor Authentication', href: show.url() }];
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <SettingsLayout>
                <div className="space-y-6">
                    ...
                    {twoFactorEnabled ? (
                        <div className="flex flex-col items-start justify-start space-y-4">
                            ...
                            <div className="relative inline">
                                <Form {...disable.form()}>
                                    {({ processing }) => (
                                        <Button color="primary-destructive" type="submit" isDisabled={processing} iconLeading={ShieldOff}>Disable 2FA</Button>
                                    )}
                                </Form>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col items-start justify-start space-y-4">
                            ...
                            <div>
                                {hasSetupData ? (
                                    <Button onClick={() => setShowSetupModal(true)} iconLeading={ShieldTick}>Continue Setup</Button>
                                ) : (
                                    <Form {...enable.form()} onSuccess={() => setShowSetupModal(true)}>
                                        {({ processing }) => (
                                            <Button type="submit" isDisabled={processing} iconLeading={ShieldTick}>Enable 2FA</Button>
                                        )}
                                    </Form>
                                )}
                            </div>
                    ...
                    <TwoFactorSetupModal
                        isOpen={showSetupModal} onClose={() => setShowSetupModal(false)} requiresConfirmation={requiresConfirmation}
                        twoFactorEnabled={twoFactorEnabled} qrCodeSvg={qrCodeSvg} manualSetupKey={manualSetupKey}
